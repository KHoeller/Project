    ...
    useEffect(() => {
        const handleClick = (evt: MapBrowserEvent<any>) => { // function for dealing with a click in the Browser 
            ...
            for (const layer of layersArray) {      // iterate over all layers 
                ...
                const isLayerQueryable = layer.get('queryable') ?? false;       // nach queryable Information suchen 
                const isLayerVisible = layer.get('visible') ?? false;           // nach visible Information suchen 
                const name = layer.get('name');                                 // nach Namen des jeweiligen Layers suchen 
                // console.log(name)
                ...
                if (isLayerQueryable && isLayerVisible) {                       // sofern layer visible = true und queryable = true ist, werden die folgenden Schritte ausgeführt
                    ...
                    // console.log(url)  
                        ...
                        if (url) {                                              // sofern die URL true ist 
                            fetch(url)
                                .then((response) => response.json())            // response auf Anfrage der URL 
                                .then((responseObject) => {
